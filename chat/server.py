...
async def exchange_for_days(exch: str, days: int):
    list_results = []
    if days >= 10:
        return 'Error: you try to put more then 10 days'

    async with aiofile.AIOFile('log_exchange.txt', 'a') as f:
        log_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        await f.write(f"{log_time}: command 'exchange' done\n")
        ...
        for exchange_rate in el['exchangeRate']:
            if exchange_rate['currency'] == exch.upper():
                try:
                    ...
                    pass
    if len(res_list) == 0:
        return f'Error: failed to search {exch.upper()}'
    else:
        a = f"{' || '.join(f'{i[0]} Sell {exch.upper()} {i[1]} Buy {exch.upper()} {i[2]}' for i in res_list)}"
        return a
    ...
    async def distribute(self, ws: WebSocketServerProtocol):
        async for message in ws:
            if message.startswith('exchange'):
                ...
                await self.send_to_clients('Hi, dear!')
            else:
                await self.send_to_clients(f"{ws.name}: {message}")
